...
export default {
  ...
  data () {
    return {
      listInstance: null,
      senderInstance: null,
      namespace: '.demo_message_list' + Math.random()
    }
  },
  computed: {},
  watch: {},
  ...
  mounted () {
    // 创建消息列表
    this.listInstance = BJY.MessageList.create({
    ...
    // 创建消息发送组件
    this.senderInstance = BJY.MessageSender.create({
  ...
  beforeDestroy () { 
    ...
    this.listInstance && this.listInstance.destroy()
    this.senderInstance && this.senderInstance.destroy()
  }
}
</script>
